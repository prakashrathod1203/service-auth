...
import om.auth.repository.FileMasterRepository;
...
public class FileMasterServiceImpl implements FileMasterService {
    ...
    public FileMasterResponse deleteById(Long id) throws ResourceNotFoundException {
        ...
        FileMasterResponse fileMasterResponse =
                CommonUtils.objectToPojoConverter(fileMaster, FileMasterResponse.class);
    ...
    @Override
    public List<FileMasterResponse> findByEntityIdAndEntityName(String id,
            EntityNameEnum entityName) throws ResourceNotFoundException {
        log.debug("No FileMasterEntity found for entityId: {} and entityName: {}", id, entityName);
        List<FileMasterEntity> fileMasters =
                fileMasterRepository.findByEntityIdAndEntityName(id, entityName);
        if (fileMasters.isEmpty()) {
            log.warn("Finding FileMasterEntity objects by entityId: {} and entityName: {}", id,
                    entityName);
        ...
        fileMasters.forEach(fileMaster -> {
            FileMasterResponse fileMasterResponse =
                    CommonUtils.objectToPojoConverter(fileMaster, FileMasterResponse.class);
            String downloadUrl =
                    String.format("%s%s", ResourceEndpoint.FILE, ResourceEndpoint.FILE_DOWNLOAD
                            .replace("{id}", String.valueOf(fileMaster.getId())));
            Objects.requireNonNull(fileMasterResponse).setDownloadUrl(downloadUrl);
            fileMasterResponses.add(fileMasterResponse);
        });
        log.debug("Found {} FileMasterEntity objects for entityId: {} and entityName: {}",
                fileMasters.size(), id, entityName);
        return fileMasterResponses;
    }

    @Override
    public List<FileMasterResponse> findByEntityIdAndEntityNames(String id,
            List<EntityNameEnum> entityNames) throws ResourceNotFoundException {
        log.debug("Entering findByEntityIdAndEntityNames with id: {} and entityNames: {}", id,
                entityNames);
        List<FileMasterEntity> fileMasters =
                fileMasterRepository.findByEntityIdAndEntityNames(id, entityNames);
        if (fileMasters.isEmpty()) {
        ...
        fileMasters.forEach(fileMaster -> {
            FileMasterResponse fileMasterResponse =
                    CommonUtils.objectToPojoConverter(fileMaster, FileMasterResponse.class);
            Objects.requireNonNull(fileMasterResponse).setDownloadUrl(
                    String.format("%s%s", ResourceEndpoint.FILE, ResourceEndpoint.FILE_DOWNLOAD
                            .replace("{id}", String.valueOf(fileMaster.getId()))));
            fileMasterResponses.add(fileMasterResponse);
        });
    ...
    @Override
    public FileMasterResponse upload(MultipartFile multipartFile, String fileName, String fileType,
            String entityId, EntityNameEnum entityName) throws IOException {
        // Build file path
        String filePathBuilder =
                storagePath + File.separator + entityId + File.separator + entityName;

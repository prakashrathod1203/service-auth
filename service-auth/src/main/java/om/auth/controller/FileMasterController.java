...
import om.auth.service.FileMasterService;
...
public class FileMasterController {
        ...
        /**
         * Handles file upload requests. This method uploads a file to the server.
         */
        @Operation(summary = SwaggerConstants.UPLOAD_FILE_SUMMARY,
                        description = SwaggerConstants.UPLOAD_FILE_DESCRIPTION,
                        tags = {SwaggerConstants.FILE_MANAGER_TAG})
        @PostMapping(value = ResourceEndpoint.FILE_UPLOAD,
        ...
        public ResponseEntity<RestApiResponse> upload(
                        ...
                        @RequestParam @Parameter(schema = @Schema(allowableValues = {"QR_CODE",
                                        "BULK_QR_CODE", "USER_PROFILE"})) EntityNameEnum entityName,
                        @RequestParam("entityId") String entityId) throws IOException {
                log.info("Received file upload request with fileName: {}, fileType: {}, entityName: {}, entityId: {}",
                                fileName, fileType, entityName, entityId);

                FileMasterResponse fileMasterResponse = fileMasterService.upload(multipartFile,
                                fileName, fileType, entityId, entityName);
                log.info("File uploaded successfully: {}", fileMasterResponse);
                RestApiResponse response =
                                new RestApiResponse(Translator.toLocale("file.upload.success"),
                                                fileMasterResponse, true);
                return ResponseEntity.status(HttpStatus.OK).body(response);
        }

        /**
         * Handles file download requests. This method download a file from the server.
         */
        @Operation(summary = SwaggerConstants.DOWNLOAD_FILE_BY_ID_SUMMARY,
                        description = SwaggerConstants.DOWNLOAD_FILE_BY_ID_DESCRIPTION,
                        tags = {SwaggerConstants.FILE_MANAGER_TAG})
        @GetMapping(value = ResourceEndpoint.FILE_DOWNLOAD,
                        consumes = MediaType.APPLICATION_OCTET_STREAM_VALUE)
        public ResponseEntity<?> download(@PathVariable(value = "id", required = true) Long id)
        ...
        /**
         * Fetches a list of files associated with a given entity. This method retrieves a list of
         * files by their entity ID and entity name.
         */
        @Operation(summary = SwaggerConstants.FETCH_FILE_LIST_BY_ENTITY_SUMMARY,
        ...
        public ResponseEntity<RestApiResponse> findByEntityId(
                        ...
                        @RequestParam(value = "entityName",
                                        required = true) EntityNameEnum entityNameEnum)
                        throws ResourceNotFoundException {
                log.info("Received request to find files with entityId: {} and entityName: {}",
                                entityId, entityNameEnum);
                List<FileMasterResponse> fileMasterResponses = fileMasterService
                                .findByEntityIdAndEntityName(entityId, entityNameEnum);
                RestApiResponse response =
                                new RestApiResponse(Translator.toLocale("operation.getList"),
                                                fileMasterResponses, true);
                return ResponseEntity.status(HttpStatus.OK).body(response);
        }

        /**
         * Deletes a file by its ID. This method deletes a file identified by its ID.
         */
        @Operation(summary = SwaggerConstants.DELETE_FILE_BY_ID_SUMMARY,
                        description = SwaggerConstants.DELETE_FILE_BY_ID_DESCRIPTION,
                        tags = {SwaggerConstants.FILE_MANAGER_TAG})
        @DeleteMapping(value = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
        public ResponseEntity<RestApiResponse> delete(@PathVariable("id") @Min(1) Long id)
                        throws ResourceNotFoundException {
                ...
                RestApiResponse response = new RestApiResponse(
                                Translator.toLocale("operation.delete"), fileMasterResponse, true);
                return ResponseEntity.status(HttpStatus.OK).body(response);
        }
}
